...
- 2026-05-08 — Added speaker-verification layer (`detectors/speaker_verify.py`). Resolves the ElevenLabs-evasion gap: even though the deepfake classifier doesn't catch ElevenLabs, comparing the caller's voiceprint against an enrolled customer baseline does. UI has a "Registered Customer Voice" upload slot (visible when Live Mode is on) and a dynamic "Registered Customer Calling" scenario.
- 2026-05-08 — Swapped speaker-verification model from Resemblyzer to SpeechBrain ECAPA-TDNN (`speechbrain/spkrec-ecapa-voxceleb`) after Resemblyzer returned 0.63 cosine similarity for a clearly different speaker (user's male voice vs ElevenLabs female). ECAPA is the industry standard, trained on VoxCeleb. Different-speaker similarities on demo files dropped from 0.43–0.54 (Resemblyzer) to -0.06–0.07 (ECAPA). Recalibrated `_combined_verdict` thresholds (sim ≥ 0.35 → 0 contribution, sim ≤ 0.25 → 0.65 FLAG-band risk). Audio loaded via librosa to avoid torchaudio's torchcodec dependency. Model files cached in `detectors/_models/` (gitignored). `requirements.txt`: dropped `resemblyzer`, added `speechbrain>=1.0` and `torchaudio>=2.2`.
- 2026-05-08 — Restructured the UI from a 2-tab pitch demo to a single-page real-time fraud-operations dashboard with human-in-the-loop. Dropped the dual-tab structure entirely (no more Live Simulation / Call Risk Audit split). New scenarios reframed as fraud-attack typology: "Authenticated Customer (Your Voice)" — only present once a reference voice is uploaded; "Voice Clone Attempt — Female Caller" (clean.mp3); "Voice Clone Attempt — Male Caller" (borderline.mp3); "Robocall Bot (Crude Synthesizer)" (ai_voice.wav). Each scenario carries caller-context metadata (claimed name, account suffix, transaction type/amount/destination, prior calls in 30d, IVR path). New HITL panel shows verdict-tinted reviewer-action card with plain-language reasoning, three action buttons (Approve & Continue, Step-Up Auth, Block & Escalate), and a session-scoped reviewer audit log. Animation window shortened from 20s to 5s for a more real-time feel. Stage-by-stage technical detail now lives in a collapsible expander beneath the dashboard. Header re-titled "VoiceGuard AI · Fraud Operations Dashboard". Without uploaded voice, the three attack scenarios all trip — speaker mismatch flags both ElevenLabs clips, deepfake + behavior signals push the robocall to BLOCK.
