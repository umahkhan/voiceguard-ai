...
def inject_css() -> None:
    css = (
        ...
        f"html,body,[class*='css']{{font-family:system-ui,-apple-system,'Segoe UI',sans-serif;color:{INK};}}"
        f".stApp{{background:{CANVAS};}}"

        # Header banner
        f".vg-header{{background:linear-gradient(135deg,{NAVY_DEEP} 0%,{NAVY} 60%,{ACCENT} 130%);"
        "padding:12px 22px;border-radius:12px;color:#fff;margin-bottom:10px;"
        "display:flex;align-items:center;justify-content:space-between;"
        "box-shadow:0 4px 14px rgba(10,61,122,0.18);}"
        ".vg-header .vg-h-title{font-size:18px;font-weight:800;letter-spacing:-0.2px;}"
        ".vg-header .vg-h-sub{font-size:11px;opacity:0.88;font-weight:500;margin-top:1px;}"
        ".vg-header .vg-h-meta{font-size:10px;opacity:0.85;font-weight:700;letter-spacing:1.4px;text-transform:uppercase;}"
        ...
        "font-size:12px;padding:6px 14px;border-radius:8px 8px 0 0;letter-spacing:0.4px;}"
        ...
        "margin-bottom:10px;box-shadow:0 4px 14px rgba(10,61,122,0.2);}"
        ".vg-incoming .vg-i-label{font-size:9px;letter-spacing:1.6px;text-transform:uppercase;"
        "color:#93c5fd;font-weight:800;}"
        ".vg-incoming .vg-i-num{font-size:20px;font-weight:800;letter-spacing:-0.3px;"
        "margin-top:3px;font-variant-numeric:tabular-nums;}"
        ".vg-incoming .vg-i-sub{font-size:10.5px;color:#cbd5e1;margin-top:1px;}"
        ...
        "border-radius:999px;font-size:9.5px;font-weight:800;letter-spacing:0.6px;}"
        ".vg-incoming .vg-i-timer-label{font-size:9px;letter-spacing:1.4px;text-transform:uppercase;"
        "color:#93c5fd;font-weight:800;margin-top:8px;}"
        ".vg-incoming .vg-i-timer{font-size:22px;font-weight:800;font-variant-numeric:tabular-nums;"
        "color:#fff;letter-spacing:-0.4px;line-height:1.1;}"

        # Waveform
        ".vg-wave{display:flex;align-items:center;gap:3px;margin-top:8px;height:30px;}"
        f".vg-wave .vg-bar{{width:4px;background:{ACCENT};border-radius:2px;"
        "animation:wave-anim 1s ease-in-out infinite;}"
        "@keyframes wave-anim{0%,100%{height:5px;}50%{height:26px;}}"

        # Verdict bar
        ".vg-verdict{border-radius:12px;padding:13px 18px;margin-bottom:10px;"
        ...
        ".vg-verdict .vg-v-label{font-size:9.5px;letter-spacing:1.7px;text-transform:uppercase;opacity:0.85;font-weight:800;}"
        ".vg-verdict .vg-v-text{font-size:24px;font-weight:800;letter-spacing:-0.4px;color:#fff;line-height:1.1;}"
        ".vg-verdict .vg-v-sub{font-size:11px;font-weight:700;opacity:0.9;letter-spacing:0.3px;}"
        ".vg-verdict .vg-v-conf-label{font-size:9.5px;letter-spacing:1.6px;text-transform:uppercase;opacity:0.8;font-weight:800;text-align:right;}"
        ".vg-verdict .vg-v-conf{font-size:26px;font-weight:800;color:#fff;text-align:right;font-variant-numeric:tabular-nums;line-height:1.1;}"

        # Risk metric card
        f".vg-metric{{background:{PAPER};border:0.5px solid {BORDER};border-radius:12px;padding:11px 13px;height:100%;}}"
        f".vg-metric .vg-m-label{{font-size:9.5px;font-weight:800;letter-spacing:1.4px;text-transform:uppercase;color:{MUTED};}}"
        f".vg-metric .vg-m-value{{font-size:24px;font-weight:800;color:{INK};font-variant-numeric:tabular-nums;line-height:1;}}"
        f".vg-metric .vg-m-desc{{font-size:10.5px;color:{MUTED};line-height:1.4;margin-top:4px;font-weight:500;}}"
        ".vg-metric .vg-m-bar{height:4px;border-radius:999px;background:#e5e7eb;margin-top:7px;overflow:hidden;}"
        ".vg-metric .vg-m-bar>span{display:block;height:100%;border-radius:999px;}"

        # Badges
        ".vg-badge{display:inline-block;padding:2px 8px;border-radius:999px;font-size:9px;font-weight:800;letter-spacing:1.1px;text-transform:uppercase;}"
        ...
        # Advisory cards
        f".vg-advisory{{background:{PAPER};border:0.5px solid {BORDER};border-radius:12px;padding:12px 14px;height:100%;}}"
        ...
        f".vg-advisory .vg-a-label{{font-size:9.5px;font-weight:800;letter-spacing:1.3px;text-transform:uppercase;color:{NAVY};}}"
        f".vg-advisory .vg-a-action{{background:#f1f5f9;border-radius:8px;padding:8px 11px;margin-top:5px;font-size:12px;font-weight:700;color:{INK};line-height:1.4;}}"
        f".vg-advisory ul{{margin:4px 0 0 0;padding-left:18px;font-size:11.5px;color:{INK};line-height:1.5;}}"
        ".vg-advisory li{margin-bottom:2px;}"
        f".vg-advisory .vg-a-narrative{{font-size:11.5px;color:{INK};line-height:1.5;margin-top:5px;font-weight:500;}}"
        ".vg-advisory .vg-a-pills{margin-top:8px;display:flex;gap:6px;flex-wrap:wrap;}"
        f".vg-advisory .vg-pill{{background:#f1f5f9;border-radius:999px;padding:3px 9px;font-size:10px;font-weight:700;color:{INK};letter-spacing:0.3px;}}"
        f".vg-advisory .vg-pill .vg-pill-k{{color:{MUTED};font-weight:600;}}"

        # Inputs
        f".stTextInput input,.stSelectbox > div > div{{border-radius:8px!important;border:1px solid {BORDER}!important;font-size:12px!important;}}"
        f".stTextInput input:focus{{border-color:{ACCENT}!important;box-shadow:0 0 0 2px rgba(55,138,221,0.18)!important;}}"

        # Button
        f".stButton > button{{background:{NAVY}!important;color:#fff!important;border:none!important;border-radius:8px!important;padding:9px 16px!important;font-size:12.5px!important;font-weight:700!important;letter-spacing:0.4px!important;width:100%;}}"
        f".stButton > button:hover{{background:{NAVY_DEEP}!important;}}"

        # Path Taken pills
        ".vg-path{display:flex;gap:6px;flex-wrap:wrap;margin-top:6px;}"
        ".vg-path .vg-step{flex:1;min-width:130px;padding:9px 10px;border-radius:999px;"
        "font-size:11px;font-weight:800;text-align:center;letter-spacing:0.3px;"
        ...
        ".vg-placeholder{background:#fff;border-radius:12px;padding:48px 28px;"
        "text-align:center;}"
        f".vg-placeholder.idle{{border:1px dashed #cbd5e1;}}"
        f".vg-placeholder.running{{border:1px dashed {ACCENT};}}"
        f".vg-placeholder .vg-p-title{{font-size:13px;font-weight:800;letter-spacing:0.2px;"
        f"color:{INK};margin-bottom:6px;}}"
        f".vg-placeholder.running .vg-p-title{{color:{NAVY};}}"
        f".vg-placeholder .vg-p-body{{font-size:11.5px;color:{MUTED};line-height:1.5;}}"
        ...
        # Stage rows
        f".vg-stage-row{{display:flex;align-items:center;gap:12px;padding:9px 12px;margin-bottom:5px;border-radius:8px;border:1px solid {BORDER};background:{PAPER};}}"
        ".vg-stage-row.skip{background:#f8fafc;border:1px dashed #cbd5e1;opacity:0.78;}"
        ".vg-stage-row .vg-s-num{width:30px;height:30px;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;flex-shrink:0;}"
        ...
        f".vg-stage-row .vg-s-name{{font-size:12.5px;font-weight:800;color:{INK};}}"
        f".vg-stage-row .vg-s-tag{{font-size:9px;color:{MUTED};letter-spacing:1px;text-transform:uppercase;font-weight:700;}}"
        ".vg-stage-row .vg-s-status{font-size:9px;padding:2px 7px;border-radius:999px;letter-spacing:1px;text-transform:uppercase;font-weight:800;}"
        ...
        f".vg-stage-row .vg-s-desc{{font-size:11px;color:{INK};margin-top:2px;line-height:1.4;font-weight:500;}}"
        f".vg-stage-row.skip .vg-s-desc{{color:{MUTED};}}"
        f".vg-stage-row .vg-s-score{{font-size:11.5px;font-weight:800;color:{INK};font-variant-numeric:tabular-nums;flex-shrink:0;min-width:130px;text-align:right;}}"
        ...
        # Audit metadata
        f".vg-meta{{background:{PAPER};border:0.5px solid {BORDER};border-radius:12px;padding:10px 13px;height:100%;}}"
        f".vg-meta .vg-meta-l{{font-size:9px;letter-spacing:1.3px;text-transform:uppercase;color:{MUTED};font-weight:800;}}"
        f".vg-meta .vg-meta-v{{font-size:18px;font-weight:800;color:{INK};font-variant-numeric:tabular-nums;margin-top:3px;line-height:1.1;}}"
        f".vg-meta .vg-meta-s{{font-size:10.5px;color:{MUTED};margin-top:3px;font-weight:600;}}"
...
def render_live_simulation() -> dict:
    ...
    with left:
        ...
        st.text_input("Caller ID", key="caller_id", on_change=_reset_call)
